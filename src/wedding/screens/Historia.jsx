...
export default function Historia() {
  ...
  function updateSection(id, patch) {
    patchContent((prev) => ({ ...prev, historia: prev.historia.map((s) => (s.id === id ? { ...s, ...patch } : s)) }))
  }
  ...
  return (
    <div style={{ paddingTop: 70 }} className="wedding-fade">
      <div style={{ padding: 'clamp(48px,7vw,96px) clamp(24px,5vw,72px) 0', textAlign: 'center' }}>
        <p style={{ margin: '0 0 16px', font: "10px/1 ui-monospace, Menlo, monospace", letterSpacing: '.26em', textTransform: 'uppercase', color: '#b0736f' }}>Capítulo uno</p>
        <EditableText
          as="h2"
          value={content.historiaHeader.title}
          onSave={(v) => patchContent((prev) => ({ ...prev, historiaHeader: { title: v } }))}
          style={{ margin: '0 0 clamp(40px,5vw,72px)', fontFamily: "'Playfair Display', serif", fontWeight: 400, fontSize: 'clamp(40px,5.6vw,80px)', color: '#3b302b' }}
        />
      </div>
      <div style={{ margin: '0 auto', maxWidth: 1100, padding: '0 clamp(24px,5vw,72px)' }}>
        <div style={{ position: 'relative', aspectRatio: '16/9', background: 'repeating-linear-gradient(135deg,#e9dcd6 0 14px,#e1d1ca 14px 28px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <PhotoSlot slotKey="hCentral" label="foto central · 2000×1125" replaceStyle={{ bottom: 12, left: 12 }} />
        </div>
      </div>
